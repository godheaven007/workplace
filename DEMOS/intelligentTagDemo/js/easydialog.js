...
Dialog.prototype = {
	...
	createOverlay : function(){
		...
		style.cssText = 'width:100%;height:100%;position:fixed;left:0;top:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9999;display:none;';
	...
	createDialogWrap : function( tmpl ){
		...
		var header = '';
		if(tmpl.header){ 	// 头部设置
			if(tmpl.noFn){	// noFn为true
				header = '<div class="easyDialog_title" id="easyDialogTitle"><a href="javascript:void(0)" title="关闭窗口" class="close_btn" id="closeBtn"></a>' + tmpl.header + '</div>';
			} else {
				header = '<div class="easyDialog_title" id="easyDialogTitle">' + tmpl.header + '</div>';
			}
		}
			// 确定按钮
		var	yesBtn = typeof tmpl.yesFn === 'function' ?
				'<span class="btn_highlight" id="easyDialogYesBtn">' + ( typeof tmpl.yesText === 'string' ? tmpl.yesText : '确&nbsp;&nbsp;定' ) + '</span>' :
				'',
			// 取消按钮	
			noBtn = typeof tmpl.noFn === 'function' || tmpl.noFn === true ? 
				'<span class="btn_normal" id="easyDialogNoBtn">' + ( typeof tmpl.noText === 'string' ? tmpl.noText : '取&nbsp;&nbsp;消' ) + '</span>' :
				'',
			// footer
			footer = yesBtn === '' && noBtn === '' ? '' :
